...
float Copter::get_surface_tracking_climb_rate_with_Hokuyo_lidar(int16_t target_rate, float current_alt_target, float dt, float* dist_err, float* target_rngfndr_alt)
{
    ...
    // reset target altitude if this controller has just been engaged
    if (now - last_call_ms > RANGEFINDER_TIMEOUT_MS) {
        target_rangefinder_alt = pos_sensor.data.pos.alt_cm + current_alt_target - current_alt;
    }
    last_call_ms = now;
    ...
    int gain = 1;
    // calc desired velocity correction from target rangefinder alt vs actual rangefinder alt (remove the error already passed to Altitude controller to avoid oscillations)
    distance_error = (target_rangefinder_alt - pos_sensor.data.pos.alt_cm) - (current_alt_target - current_alt);
